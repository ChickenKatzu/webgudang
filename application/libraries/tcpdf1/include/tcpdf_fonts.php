...
class TCPDF_FONTS
{
	 ...
	 * @public static
	 */
	public static function UTF8ToUTF16BE($str, $currentfont, $isunicode = true, $setbom = true)
	{
		if (!empty($isunicode)) {
	 ...
	 * @public static
	 */
	public static function utf8StrRev($str, $currentfont, $setbom = true, $forcertl = false, $isunicode = true)
	{
	 ...
	 * @public static
	 */
	public static function utf8StrArrRev($arr, $str, $currentfont, $setbom = true, $forcertl = false, $isunicode = true)
	{
	...
	public static function utf8Bidi($ta, $currentfont, $str, $forcertl = false, $isunicode = true)
	{
		...
		$ordarray = array();
		foreach ($chardata as $cd) {
			// Check if $cd is an array and contains the 'char' key
			if (is_array($cd) && isset($cd['char'])) {
				$ordarray[] = $cd['char'];  // Store the char value
				// Store char values for subsetting
				$currentfont['subsetchars'][$cd['char']] = true;
			} elseif (is_string($cd)) {
				// If $cd is a string, treat it directly
				$ordarray[] = $cd;  // Add the string directly to $ordarray
				// Store the string for subsetting
				$currentfont['subsetchars'][$cd] = true;
			} else {
				// Handle other cases, if necessary
				continue;  // Skip if $cd is neither an array nor a string
			}
		}
		return $ordarray;
	}
